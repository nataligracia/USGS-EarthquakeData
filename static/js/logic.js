...
var grayMap = L.titleLayer("https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}", {
...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/geojson.php").then(function(data)) {
  ...
  });  // use .addTo to add the L.geoJson object to the earthquakes LayerGroup

  // Then we add the earthquake layer to our map.
  earthquakes.addTo(map); // use .addTo to add the earthquakes LayerGroup to the myMap object
  ...
  d3.json("https://github.com/fraxen/tectonicplates/blob/master/GeoJSON/PB2002_boundaries.json").then(function(platedata) {
      ...
      tectonicplates.addTo(map); // use .addTo to add the tectonicPlates LayerGroup to the myMap object
    });
};
